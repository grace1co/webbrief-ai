  ...
  return (
    <Shell>
      <div className="max-w-[1000px] mx-auto px-gutter py-stack-lg flex flex-col gap-stack-md">
        <div className="flex items-center justify-between gap-stack-md flex-wrap"><div><span className="text-label-caps uppercase text-[rgb(0_194_235)] dark:text-secondary">History</span><h1 className="text-h2 font-bold mt-1">Recent Analyses</h1><p className="text-body-sm text-on-surface-variant">History is automatic. Bookmark analyses you want to keep permanently.</p></div><Button onClick={() => navigate("/")}><Icon name="add" /> New Analysis</Button></div>

        <Card className="flex items-end justify-between gap-stack-md flex-wrap">
          <label className="flex flex-col gap-stack-sm"><span className="text-body-sm font-bold">History retention</span><select value={settings?.retention_days ?? "forever"} onChange={(e) => updateRetention(e.target.value)} disabled={!settings} className={`rounded-lg px-stack-md py-stack-sm text-body-sm disabled:opacity-60 ${fieldClassName}`}><option value="7">Keep history for 7 days</option><option value="30">Keep history for 30 days</option><option value="90">Keep history for 90 days</option><option value="forever">Keep history forever</option></select></label>
          <button onClick={clearHistory} className="inline-flex items-center gap-stack-sm rounded-lg border border-slate-300 px-stack-md py-stack-sm text-body-sm font-bold text-slate-600 hover:border-red-400 hover:text-red-600 dark:border-outline-variant dark:text-on-surface-variant dark:hover:border-error dark:hover:text-error"><Icon name="delete_sweep" /> Clear History</button>
        </Card>
        ...
        {items && items.length > 0 && <div className="relative"><Icon name="search" className="absolute left-stack-sm top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none dark:text-on-surface-variant" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search history..." className={`w-full rounded-lg pl-10 pr-stack-md py-stack-sm text-body-md focus:outline-none focus:border-primary ${fieldClassName}`} /></div>}
